...
void store_product_decrease_stock(Store* store, Product* product, int del)
{
    if (product->quantity < del){
        printf("Impossible de diminuer le stock car celui-ci est trop bas!\n");
        sleep(2);
        return;
    }
    product->quantity -= del;
    store->currentCapacity -= product->size * del;
}
